...
// تم تحديث البيانات بالحسابات الشخصية الخاصة بك
const SOCIALS = [
  { 
    name: "LinkedIn", 
  ...
  { 
    ...
    color: "#ffffff", // أبيض ليناسب الـ Dark Mode
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative py-32 px-16 bg-surface overflow-hidden border-b border-border">
      ...
      <Reveal className="flex items-center gap-5 mb-20 relative z-10">
        <span className="font-mono text-[0.65rem] text-accent tracking-[0.3em]">04</span>
        <div className="h-px w-24 bg-gradient-to-r from-accent/50 to-transparent" />
        <h2 className="font-bebas text-5xl tracking-widest text-white">Start a Conversation</h2>
      </Reveal>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10 max-w-7xl mx-auto">
        
        {/* Calendar / Booking System */}
        <Reveal direction="left">
          <div className="clip-both border border-border bg-[#0a0a0a]/50 backdrop-blur-md p-10 relative overflow-hidden shadow-2xl">
            {/* Top Accent Line */}
            ...
            <div className="flex items-center justify-between mb-8">
              <span className="font-syne font-semibold text-2xl text-white tracking-wide">April 2026</span>
              <div className="flex gap-2">
                {["‹", "›"].map((ch) => (
                  <motion.button
                    key={ch}
                    whileHover={{ scale: 1.05, borderColor: "#C88A60", color: "#C88A60" }} // Accent color
                    whileTap={{ scale: 0.95 }}
            ...
            <div className="grid grid-cols-7 gap-1 mb-3">
              {DAYS.map((d) => (
                <div key={d} className="font-mono text-[0.6rem] text-muted text-center py-1 tracking-widest uppercase">
                  {d}
                </div>
              ))}
            </div>
              ...
              {cells.map((day, i) =>
                day === null ? (
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="aspect-square flex items-center justify-center font-mono text-[0.75rem] relative transition-all border border-transparent"
                    style={{
                      background: selected === day ? "#C88A60" : "transparent", // Accent color
                      color: selected === day ? "#0B0A0A" : AVAILABLE.includes(day) ? "#E6D5B8" : "#4b5a6e", // Dark text on selected
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {SLOTS.map((s) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="font-mono text-[0.65rem] tracking-wider px-4 py-2 border transition-all"
                  style={{
            ...
            <AnimatePresence mode="wait">
              {!booked ? (
                <motion.div key="form" className="flex flex-col gap-3">
                  {[
                    ...
                    <motion.input
                      ...
                      className="px-5 py-4 bg-bg border border-border text-white font-mono text-[0.75rem] outline-none placeholder-muted transition-colors focus:bg-surface"
                    />
                  ))}
                  <motion.button
                    onClick={handleBook}
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="py-4 bg-accent text-bg font-mono font-bold text-[0.7rem] tracking-widest uppercase mt-2 transition-colors"
                  >
                ...
                <motion.div
                  ...
                  className="text-center py-10 bg-bg border border-border"
                >
                  <div className="text-accent mb-4">
                    <svg className="w-12 h-12 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                  </div>
                  <div className="font-syne font-bold text-xl text-white mb-2">Meeting Confirmed</div>
                  <div className="font-mono text-[0.65rem] tracking-wider text-muted">
        ...
        <div className="flex flex-col gap-4 justify-center">
          {SOCIALS.map((s, i) => (
            <Reveal key={s.name} delay={i * 0.1} direction="right">
              <motion.a
                ...
                whileHover={{ x: 8 }}
                className="flex items-center gap-5 p-5 border border-border bg-[#0a0a0a]/30 relative overflow-hidden group transition-all duration-300 hover:border-accent/30"
              >
                {/* Top Animated Bar */}
                <motion.div
                ...
                {/* SVG Icon */}
                <div
                  className="w-12 h-12 bg-surface border border-border flex items-center justify-center flex-shrink-0 group-hover:bg-white/5 transition-all duration-300"
                  style={{ color: s.color }}
                >
                  {s.svg}
                </div>
                ...
                <div className="flex-1">
                  <div className="font-syne font-semibold text-white text-base mb-1 tracking-wide">{s.name}</div>
                  <div className="font-mono text-[0.65rem] text-muted tracking-wider">{s.handle}</div>
                </div>
          ...
          <Reveal delay={0.5} direction="right">
            <div className="p-6 bg-surface border border-border mt-4 flex items-center justify-between">
              <div>
                <div className="font-mono text-[0.6rem] text-accent tracking-[0.2em] uppercase mb-2">
                  Typical Response
                </div>
                <div className="font-bebas text-3xl text-white tracking-widest">Under 12 Hours</div>
              </div>
